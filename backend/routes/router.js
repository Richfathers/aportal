...
export const updateDevicePage = async (req,res) =>{
  try {
      ...
      const deviceData = await axios.get(`http://localhost:3000/device/one-device/${idishnik}`)
...
export const deleteDevice =  async (req, res ) =>{
  try {
    ...
    const deleted  = await axios.get(`http://localhost:3000/device/delete-device/${idishnik}`)
    console.log(deleted)
  } catch (error) {
    console.log(error)
  }
